...
* 
*	@author Jonas de Luna Ieno, Pedro Granville e Vinícius Roosvelt.
* 
*	@version 1.00
*/
...
XE910::XE910(Uart* uartModule):at(uartModule) {
...
*/
bool XE910::Init(const char* operadora, const char* ip) {
    bool result = false;
...
*	@return Retorna 1 se receber resposta "ok" do módulo e 0 se receber "error".
*/
bool XE910::Connect(char* resp, int respLen) {
    bool result      = false;
    char command[64] = {0};
...
*	@return Retorna 1 se receber resposta "ok" do módulo e 0 se receber "error".
*/
bool XE910::SocketConnect(char* address, int port) {
...
*	@return Retorna 1 se receber resposta "ok" do módulo e 0 se receber "error".
*/
bool XE910::SocketWrite(char* buffer) {
    char command[64] = {0};
	bool result      = false;
	char toSend[200] = {0};
	char     resp[100] = {0};
...
*/
int  XE910::SocketRead(char* buffer, uint16_t maxBufferLen, uint16_t timeout) {
    bool result = false;
...
*	@return Retorna 1 se receber resposta "ok" do módulo e 0 se receber "error".
*/
bool XE910::SocketDisconnect(void) {
    bool result = false;
	char command[64] = {0};
...
*	@return Retorna 1 se receber resposta "ok" do módulo e 0 se receber "error".
*/
bool XE910::CloudConnect(char* apiServer, char* appToken) {
...
*	@return Retorna 1 se receber resposta "ok" do módulo e 0 se receber "error".
*/
bool XE910::Send2Cloud(char* variable, char* value, uint16_t timeout) {
    char  command[64] = {0};
	bool  result      = false;
	char	 receive[100] = {0};
	char    resp[100] = {0};
...
*	@return Retorna 1 se receber resposta "ok" do módulo e 0 se receber "error".
*/
bool XE910::CloudDisconnect(void) {
    bool result = false;
	char command[64] = {0};
...
*	@return Retorna 1 se receber resposta "ok" do módulo e 0 se receber "error".
*/
bool XE910::MessageWrite(const char* number, char* buffer) {
    char command[64] = {0};
	bool result      = false;
	char toSend[200] = {0};
	char     respSend[100] = {0};
...
*	@return Retorna 1 se receber resposta "ok" do módulo e 0 se receber "error".
*/
bool  XE910::MessageRead(char* buffer, uint16_t timeout) {
    bool result = false;
...
*	@return Retorna 1 se receber resposta "ok" do módulo e 0 se receber "error".
*/
bool XE910::GetFixGPS(char* time, double* latitude, double* longitude, char* satelites, char* altitude) {
	...
	if((at.Read(receive, 300, 1000, 500)) >= 70){
...
*	@return Retorna 1 se receber resposta "ok" do módulo e 0 se receber "error".
*/
bool XE910::DoACall(char* number) {
	bool result = false;
	char command[64] = {0};
...
*	@return Retorna 1 se receber resposta "ok" do módulo e 0 se receber "error".
*/
bool XE910::GetFixLBS(double* latitude, double* longitude) {
	bool result = false;
	...
	strcpy(command, CMD_GSM_FIX);
	
	do{	
		result = AT_RESP_OK == at.SendATCmdReturnResp(command, 5000, 100, receive, 100, NUM_TRIES);
		delay(100);
	}while((!strstr(receive, "#AGPSRING:")) && (time-->0));
	...
	if (result) {
		...
		*longitude = atof(StrLongitude);
		...
		if ((strlen(StrLatitude) == 0) || (strlen(StrLongitude) == 0))
			result = false;
...
*	@return Retorna 1 se receber resposta "ok" do módulo e 0 se receber "error".
*/
bool XE910::Fix2Cloud(char* coordenada, char* valor, uint16_t timeout) {
    char  command[64] = {0};
	bool  result      = false;
	char	 receive[100] = {0};
	char    resp[100] = {0};
	...
	strcpy(command, CMD_GSM_SEND2CLOUD);
    strcat(command, "=0,property.publish,key,");
	strcat(command, coordenada);
	strcat(command, ",value,");
	strcat(command, valor);
...
*	@return Retorna 1 se receber resposta "ok" do módulo e 0 se receber "error".
*/	
bool XE910::PlayAudio(char* file) 	//name = nome do arquivo de audio com o formato, armazendado na memoria interna do modulo
{
	bool result = false;
	char command[64] = {0};
...
*	@return Retorna 1 se receber resposta "ok" do módulo e 0 se receber "error".
*/	
bool XE910::RingAutoAnswer(int nRING) 	
{
	bool result = false;
	char command[64] = {0};
	char num[2] = {0};
